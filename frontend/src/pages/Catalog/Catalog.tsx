...
import SortingOrderMedia, {
	OptionType
} from '../../components/Filters/Media/SortingOrderMedia/SortingOrderMedia';
...
function Catalog({ className, isAdmin = false }: CatalogProps) {
	...
	const { products: productsGetting, isLoading } = useAppSelector(
	...
	const [deleteProductsClicked, setDeleteProductsClicked] =
	...
	useEffect(() => {
		if (
			...
			dispatch(
				fetchCharacteristic({
					category: catParam,
					search: searchResults as string
				})
			);
			...
			dispatch(
				fetchProduct({
					category:
						isAdmin && categoryFilters ? categoryFilters : catParam,
					sortBy: sortBy,
		...
		if (
			...
			charsParam.length === 0 &&
			!sortBy &&
			((catParam && currentCategory !== catParam) ||
		...
		<section>
			...
			<div className={cn(styles['catalog'], className)}>
				{products.length > 0 || isAdmin ? (
						...
						<div className={cn(styles['catalog-body'])}>
							<div className={cn(styles['product-items'])}>
								<ProductList
									...
									removeProductDelete={removeProductDelete}
								...
								{isAdmin && products.length > 0 && (
									<div>
										<ItemsSelector
											onChangeOption={onChangeItemsCount}
											defaultOption="Товаров на странице"
											optionLabels={[
												{ id: 10, name: '10' },
												{ id: 50, name: '50' },
												{ id: 100, name: '100' }
					...
					<PageHead>Товары не найдены</PageHead>
				)}
			</div>

			{isAdmin && (
				<ModalAskDelete
					isOpen={deleteProductsClicked}
					closeModal={onClickDeleteProducts}
					message={'выбранные товары'}
					idsItems={productDeleteIds}
