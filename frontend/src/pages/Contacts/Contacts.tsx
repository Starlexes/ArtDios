...
function Contacts({ className }: ContactsProps) {
	...
	const openTime =
		String(workingHours.openingHours).length < 2
			? '00:00'
			: String(workingHours.openingHours).slice(0, 5);
	const closeTime =
		String(workingHours.closingHours).length < 2
			? '00:00'
			: String(workingHours.closingHours).slice(0, 5);
		...
		isLoading ? (
			...
			<section>
				<div className={cn(styles['contacts'], className)}>
					...
					<PageHead>Контакты</PageHead>
					<ContactsItems>
						<ContactsItem>
							<ContactsImage>{phoneContacts()}</ContactsImage>
							<ContactsContent
								className={cn(styles['phones-content'])}
							>
								{phones.length > 0 && (
									<AnchorNavItem
										href={`tel:${phones[0]}`}
						...
						<ContactsItem>
							<ContactsImage>{mailContacts()}</ContactsImage>
							<ContactsContent>
								<AnchorNavItem
									href={`mailto:${emails.length > 0 ? emails[0] : ''}`}
						...
						<ContactsItem>
							<ContactsImage>{addressContacts()}</ContactsImage>
							<ContactsContent>
						...
						<div className={cn(styles['bottom-item'])}>
							<ContactsItem className={cn(styles['social-item'])}>
								...
								<ContactsContent
									className={cn(styles['social-content'])}
								>
							...
							<ContactsItem className={cn(styles['hours-item'])}>
								<ContactsImage>{hoursContacts()}</ContactsImage>
								<ContactsContent
									className={cn(styles['hours-content'])}
								>
									<span>Время работы:</span>
									<span>
										c {openTime} до {closeTime}
									</span>
								</ContactsContent>
							</ContactsItem>
						</div>
