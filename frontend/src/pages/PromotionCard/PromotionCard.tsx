...
function PromotionCard({ className }: PromotionCardProps) {
	...
	const {
		...
		error: promoError,
		isLoading
	} = useAppSelector((state: RootState) => state.promotionCard);

	const DisplayContent = (htmlContent: string) => {
		const cleanHTML = DOMPurify.sanitize(htmlContent);
		return (
			<div
				className={cn(styles['promotion-desc'])}
			...
			<section>
				<div className={cn(styles['promotion'], className)}>
					...
					<div className={cn(styles['promotion-content'], className)}>
						<img
							src={mediaImagesPath + '/other/promo-cover.png'}
							alt="Promo cover"
							className={cn(styles['promotion-cover'])}
						/>
