...
export const fetchCharacteristic = createAsyncThunk<
	...
	CharParams,
	{ rejectValue: string }
>('characteristic/fetchCharacteristic', async (params, { rejectWithValue }) => {
	try {
...
export const updateCharacteristic = createAsyncThunk<
	...
	{ rejectValue: string }
>('characteristic/updateCharacteristic', async (data, { rejectWithValue }) => {
	try {
...
export const addCharacteristic = createAsyncThunk<
	Characteristic,
	UpdateCharacteristic | UpdateCharacteristic[],
	{ rejectValue: string }
>('characteristic/addCharacteristic', async (data, { rejectWithValue }) => {
	try {
...
export const deleteCharacteristic = createAsyncThunk<
	...
	number,
	{ rejectValue: string }
>('characteristic/deleteCharacteristic', async (id, { rejectWithValue }) => {
	try {
...
const charSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(
			fetchCharacteristic.fulfilled,
			(state, action: PayloadAction<Characteristic[]>) => {
		...
		builder.addCase(
			updateCharacteristic.fulfilled,
			(state, action: PayloadAction<Characteristic>) => {
				state.chars = state.chars.map((item) =>
					item.char_id === action.payload.char_id
						? action.payload
						: item
				);
			}
		...
		builder.addCase(
			deleteCharacteristic.fulfilled,
			(state, action: PayloadAction<number>) => {
				state.chars = state.chars.filter(
					(item) => item.char_id !== action.payload
				);
			}
		);
		builder.addCase(
			addCharacteristic.fulfilled,
			(state, action: PayloadAction<Characteristic>) => {
