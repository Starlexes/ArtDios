...
export const fetchPopProduct = createAsyncThunk<
	PopularProduct[],
	void,
	{ rejectValue: string }
>('popProduct/fetchPopProduct', async (_, { rejectWithValue }) => {
	try {
...
export const updatePopularProduct = createAsyncThunk<
	PopularProduct,
...
export const addPopularProduct = createAsyncThunk<
	...
	FormData,
	{ rejectValue: string }
>('popProduct/addPopularProduct', async (data, { rejectWithValue }) => {
	try {
...
export const deletePopularProduct = createAsyncThunk<
	...
	number,
	{ rejectValue: string }
>('popProduct/deletePopularProduct', async (id, { rejectWithValue }) => {
	try {
...
const popProductSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(
			fetchPopProduct.fulfilled,
			(state, action: PayloadAction<PopularProduct[]>) => {
		...
		builder.addCase(
			updatePopularProduct.fulfilled,
			(state, action: PayloadAction<PopularProduct>) => {
				state.popProducts = state.popProducts.map((item) =>
		...
		builder.addCase(
			...
			(state, action: PayloadAction<number>) => {
				state.popProducts = state.popProducts.filter(
