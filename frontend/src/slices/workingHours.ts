...
export const fetchWorkingHours = createAsyncThunk<
	...
	void,
	{ rejectValue: string }
>('workingHours/fetchWorkingHours', async (_, { rejectWithValue }) => {
	try {
...
export const addWorkingHours = createAsyncThunk<
	...
	UpdateWorkingHours,
	{ rejectValue: string }
>('workingHours/addWorkingHours', async (data, { rejectWithValue }) => {
	try {
...
const workingHoursSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(
			updateWorkingHours.fulfilled,
			(state, action: PayloadAction<WorkingHours>) => {
