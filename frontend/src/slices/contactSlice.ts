	...
	extraReducers: (builder) => {
		...
		builder.addCase(fetchContacts.fulfilled, (state, action) => {
			state.contacts.emails =
				action.payload.emails.length > 0
					? action.payload.emails.map((item) => item.email)
					: [];
			state.contacts.phones =
				action.payload.phones.length > 0
			...
			state.contacts.workingHours =
				action.payload.working_hours.length > 0
					? {
							...action.payload.working_hours[0],
							openingHours:
								action.payload.working_hours[0].opening_hours,
							closingHours:
								action.payload.working_hours[0].closing_hours
						}
					: {
							openingHours: 0,
							closingHours: 0
...
export const { setPhones, setEmails, setAddresses, setWorkingHours } =
