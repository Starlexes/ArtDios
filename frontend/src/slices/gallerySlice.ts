...
export const updateGallery = createAsyncThunk<
	...
	UpdateGalleryState,
	{ rejectValue: string }
>('gallery/updateGallery', async ({ id, data }, { rejectWithValue }) => {
	try {
...
export const addGallery = createAsyncThunk<
	...
	FormData,
	{ rejectValue: string }
>('gallery/addGallery', async (data, { rejectWithValue }) => {
	try {
...
export const deleteGallery = createAsyncThunk<
	...
	number,
	{ rejectValue: string }
>('gallery/deleteGallery', async (id, { rejectWithValue }) => {
	try {
		...
		builder.addCase(
			...
			(state, action: PayloadAction<Gallery>) => {
				state.gallery = state.gallery.map((item) =>
					item.gallery_id === action.payload.gallery_id
						? action.payload
						: item
				);
			}
		...
		builder.addCase(
			...
			(state, action: PayloadAction<number>) => {
				state.gallery = state.gallery.filter(
					(item) => item.gallery_id !== action.payload
				);
			}
		);
		builder.addCase(
			addGallery.fulfilled,
			(state, action: PayloadAction<Gallery>) => {
