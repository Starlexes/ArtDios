...
function ModelEditItem({
	...
	const onClickPromo = () => {
		...
		'discount' in modelItem &&
			modelItem.discount &&
			onClickDiscount &&
			onClickDiscount(modelItem.id, null);
	};
	...
	return (
		...
		>
			{editClicked ? (
				<ModelEditInput value={inputValue} onChange={onChange} />
			) : (
				modelItem.name
			)}
			...
			<ModelEditActions>
				{'discount' in modelItem &&
					(promoClicked && !modelItem.discount ? (
						<AddPromoField
							onClickAdd={onClickPromo}
							onChangePromo={onChangePromo}
							onClickSubmit={onClickAcceptPromo}
						/>
					) : (
						<AddPromoButton onClick={onClickPromo}>
							{modelItem.discount ? (
								`Убрать (${modelItem.discount}%)`
							) : (
								<>
									Скидка
									{addPlus()}
