...
function ItemCardCharacteristic({
	className,
	orderNumber,
	charItem,
	acceptClicked = false,
		...
		<ItemActions>
			<div className={cn(styles['item-card'], className)}>
				...
				<div className={cn(styles['char-item'])}>
					...
					<ItemCardInput
						...
						value={existChar && !name ? charItem?.property : name}
			...
			<AddItemButton
				...
				onClick={() => {
					...
					charItem?.new &&
						onClickDelete &&
						onClickDelete(Number(charItem?.id));
				}}
			>
				{removeItemMinus()}
			</AddItemButton>
			...
			<ModalAskDelete
				isOpen={existChar && isDeleteClicked}
				closeModal={() => setIsDeleteClicked(!isDeleteClicked)}
				message={`характеристику "${charItem?.property} - ${charItem?.value}"`}
				idItem={charItem?.id}
