	...
	return (
		<CardEditItemActions>
			<div className={cn(styles['card-edit'], className)}>
				{editClicked || newItem ? (
					<ItemCardInput
						...
						value={inputValue}
						className={cn(styles['content-input'])}
						errors={isErrors}
						type={type}
					/>
				) : (
				...
				{editClicked || newItem ? (
					<ModelAcceptButton
						onClick={() => {
							inputValue &&
								onClickAccept &&
								onClickAccept(idItem, inputValue);
			...
			<AddItemButton
				...
				onClick={newItem ? onRemoveItem : onClickDelete}
			>
				{removeItemMinus()}
			</AddItemButton>

			<ModalAskDelete
				isOpen={newItem ? false : deleteClicked}
				closeModal={onClickDelete}
				message={String(deleteMessage)}
				idItem={idItem}
				onDelete={onDelete}
			/>
		</CardEditItemActions>
