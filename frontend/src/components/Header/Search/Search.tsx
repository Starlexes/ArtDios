...
function Search({ className, isAdmin = false }: SearchProps) {
	...
	const highlightText = (text: string) => {
		...
		return parts.map((part, index) =>
			regex.test(part) ? (
				<span key={index} className={cn(styles['matches'])}>
					{part}
				</span>
			) : (
				part
	...
	return (
		<>
			...
			>
				...
				/>
				<Button
					className={cn(styles['search-btn'])}
					onClick={onClickSearch}
				>
					<img
						src={mediaImagesPath + '/other/loupe.svg'}
						alt="Иконка лупы"
					/>
				</Button>
			...
			{!isAdmin &&
				...
				inputValue && (
					<div
						className={cn(styles['search-overlay'])}
						onClick={() => setSuggestsActive(false)}
					></div>
				)}
		</>
	);
