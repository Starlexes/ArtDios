...
function CartsDoor({ className }: CartsDoorProps) {
	...
	return matches ? (
		<div className={cn(styles['cart-doors'], className)}>
			<CartDoorItem>Межкомнатные двери купе</CartDoorItem>

			<CartDoorItem>Межкомнатные двери складные</CartDoorItem>

			<CartDoorItem>Межкомнатные двери гармошка</CartDoorItem>

			<CartDoorItem>Межкомнатные скрытые двери</CartDoorItem>

			<CartDoorItem>Межкомнатные двери без краски</CartDoorItem>
		</div>
	) : (
		<SwiperItem
			slidesPerView={!mobileMatches ? 1 : secondMatches ? 3 : 2}
			betweenSlider={!mobileMatches ? 80 : 0}
		>
