...
function PriceFilter({
	...
	useEffect(() => {
		...
		dispatch(
			setSubmitFilterParams({ maxPrice: maxPrice, minPrice: minPrice })
		);
	...
	useEffect(() => {
		if (isClicked) {
			...
			dispatch(
				setSubmitFilterParams({ maxPrice: maxVal, minPrice: minVal })
			);
		}
	...
	return (
		<div className={cn(styles['price-filter'], className)}>
			<PropertyTitle>Цена</PropertyTitle>
			...
			<ReactSlider
				...
				renderTrack={(props) => {
					const percent1 =
						((Number(minPrice) - Number(minVal)) /
							(Number(maxVal) - Number(minVal))) *
						100;
					const percent2 =
						((Number(maxPrice) - Number(minVal)) /
							(Number(maxVal) - Number(minVal))) *
						100;
			...
			<div className={cn(styles['price-input'])}>
				<Input
					...
					className={cn(styles['price'], styles['min-price'])}
					type="number"
					value={minPrice}
					name="min-price"
					onBlur={onBlur}
				...
				<Input
					onInput={handleChange}
					className={cn(styles['price'], styles['max-price'])}
					type="number"
					value={maxPrice}
					name="max-price"
					onBlur={onBlur}
