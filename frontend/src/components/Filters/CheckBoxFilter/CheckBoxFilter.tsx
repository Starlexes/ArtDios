...
function CheckBoxFilter({
	...
	const { chars, category } = useAppSelector(
		(state: RootState) => state.buttons.actionSubmitButton.filterparams
	);

	const descExists = isCategory
		? category?.find((item) => item === desc)
		: chars?.find((item) => item.description === desc);
	const initial = descExists ? true : false;

	const [isChecked, setIsChecked] = useState<boolean>(initial);

	const { isClicked: toClear } = useAppSelector(
		(state: RootState) => state.buttons.actionClearButton
	);
	...
	const onChange = (e: ChangeEvent<HTMLInputElement>) => {
		...
		const exists = chars?.some(
			(obj) => obj.name === propertyName && obj.description === desc
		);

		if (isCategory) {
			if (descExists && !checked) {
				const updatedArray = category?.filter((obj) => !(obj === desc));

				dispatch(setSubmitFilterParams({ category: updatedArray }));
			} else if (checked) {
				const newArray = [...(category ?? []), desc];
	...
	return (
		<div className={cn(styles['checkbox-filter'], className)}>
			...
			<label className={cn(styles['checkbox-label'])}>
				<span
					className={cn(styles['checkbox-desc'], {
