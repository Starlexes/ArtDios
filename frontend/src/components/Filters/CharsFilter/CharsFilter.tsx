...
function CharsFilter({
	...
	const { isClicked, filterparams } = useAppSelector(
		(state: RootState) => state.buttons.actionSubmitButton
	);
	...
	const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
		...
		if (isClicked) {
			...
			const convertedCats =
				isAdmin && catParam && catParam.length > 0
					? new URLSearchParams(
			...
			const priceRoute =
				productLength > 1
					? `${searchRoute ? '&' : ''}min-price=${minPriceParam}&max-price=${maxPriceParam}`
					: '';
			navigate(
				`${url}?${searchRoute}${priceRoute}${convertedCats ? `&${convertedCats}` : ''}${charsParam?.length ? `&${convertedChars}` : ''}`
			);
		...
		<aside>
			<div className={cn(styles['main-filter'], className)}>
				{!mediaMatches && (
					<Button
						onClick={closeModal}
						className={cn(styles['close-filters'])}
					>
						{closeFilters()}
					</Button>
				)}
				...
				<form
					...
					className={cn(styles['filters-form'])}
				>
					{productLength > 1 && (
						<PriceFilter
							minVal={minPrice}
