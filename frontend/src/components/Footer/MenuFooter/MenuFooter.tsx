...
function MenuFooter({ className, ...props }: MenuFooterProps) {
	const { contacts, isLoading: isLoadingContacts } = useSelector(
	...
	return (
		<div className={cn(styles['menu-footer'], className)} {...props}>
				...
				{categories.length > 0 &&
					categories.map((item) => (
						<NavItem
							...
							key={item.name}
							className={cn(styles['contact-text'])}
						>
							{item.name}
						</NavItem>
					))}
			...
			<MenuFooterItem>
				...
				<NavItem
					to={deliveryPaymentsRoute}
					className={cn(styles['contact-text'])}
				>
					Доставка и оплата
				</NavItem>
				<NavItem
					to={deliveryPaymentsRoute + '#returning-product'}
					isHashLink={true}
					className={cn(styles['contact-text'])}
				>
					Возврат товара
			...
			<MenuFooterItem>
				...
				<AnchorNavItem
					href="https://wa.me/+79782111387?text=Здравствуйте"
					className={cn(styles['contact-text'])}
					target="blank"
				>
					<FooterSocialItem>
						<img
							src={mediaImagesPath + '/social/whatsapp logo.svg'}
				...
				<AnchorNavItem
					href="viber://chat?number=+79782111387"
					className={cn(styles['contact-text'])}
					target="blank"
				>
					<FooterSocialItem>
						<img
							src={mediaImagesPath + '/social/viber logo.svg'}
							alt="Логотип Viber"
						/>
						Viber
					</FooterSocialItem>
				...
				<AnchorNavItem
					href="https://t.me/+79782111387"
					className={cn(styles['contact-text'])}
					target="blank"
				>
					<FooterSocialItem>
						<img
							src={mediaImagesPath + '/social/telegram logo.svg'}
