...
function CreateProduct({ className, newItem = false }: CreateProductProps) {
	...
	const { productTypes, isLoading: isLoadingProductType } = useAppSelector(
	...
	const { product: productCard, isLoading: isLoadingProductCard } =
		useAppSelector((state: RootState) => state.productCard);
	...
	const onClickAdd = () => {
		...
		setChars((chars) => [
			...chars,
			{ property: '', value: '', id: uniqueId, new: true }
		]);
	};
	...
	const onClickSubmit = () => {
		...
		if (
			productParam &&
			chars.every((item) => item.property && item.value)
		) {
			...
			dispatch(
				updateProductCard({
					id: Number(productCard?.product_id),
					data: formData
				})
			);
			updatedChars.length > 0 &&
				dispatch(
					updateCharacteristic(
						updatedChars.map((item) => {
							return {
								char_id: item.id,
								name: item.property,
					...
					addCharacteristic(
						newItems.map((item) => {
							return {
								name: item.property,
								description: item.value,
								product: productCard?.product_id
							};
						})
					)
				);
			...
			if (
				[name, image, description, code, price, subcategory].every(
					(item) => item
				)
			) {
				const formData = new FormData();
	...
	useEffect(() => {
		if (
			...
			productParam &&
			productCard &&
			productCard.characteristics.length > 0
		) {
	...
	return (
		<section>
			{isLoadingProductCard || !isFetched ? (
				<Spinner />
			) : productCard || newItem ? (
				<>
					<HelmetProvider>
					...
					<NewItemLayout
						dark={false}
						className={cn(styles['products-items'], className)}
					>
						<div className={cn(styles['main-info'])}>
							<ItemCardInputArea
								dark={true}
								className={cn(styles['input-small'])}
							>
								<ItemCardInputLabel dark={false}>
									Код товара:
								</ItemCardInputLabel>
								<ItemCardInput
									dark={false}
									onChange={onChangeCode}
									errors={acceptClicked && !code}
							...
							<div className={cn(styles['info-part'])}>
								...
								<div className={cn(styles['images'])}>
									...
									<ItemImagePreview
										dark={true}
										image={
											productParam && !secondImage
												? productCard?.second_image
												: secondImage
										}
										onChange={onChangeSecondImage}
										title="Допольнительное изображение 1:"
										imageClassName={cn(
											styles['second-image']
										)}
									/>
									<ItemImagePreview
										dark={true}
										image={
											productParam && !thirdImage
												? productCard?.third_image
												: thirdImage
										}
										onChange={onChangeThirdImage}
										title="Допольнительное изображение 2:"
										imageClassName={cn(
											styles['second-image']
										)}
									/>
							...
							<div className={cn(styles['info-part'])}>
								...
								{subcategories.length > 0 && (
									<ItemsSelector
										...
										onChangeOption={onChangeSubCategory}
										selectErrors={
											acceptClicked && !subcategory
										}
										small={true}
									/>
								)}
								...
								<ItemCardCharacteristic
									...
									onChangeChars={
										typeof item.update === 'boolean'
											? onChangeUpdatedChars
											: onChangeChars
									}
									onClickDelete={onClickDelete}
									existChar={typeof item.update === 'boolean'}
						...
						<ModalAskDelete
							...
							closeModal={onClickDeleteProduct}
							message={`продукт "${productCard?.name}"`}
							idItem={productCard?.product_id}
