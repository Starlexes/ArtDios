		...
		categories.length > 0
			? categories.filter(
					(category) =>
						...
						) || category.id === popItem?.category
				)
			: [];

	const popCategory =
		validatedCategories.length > 0
			? validatedCategories.find((item) => item.id === popItem?.category)
			: null;
	...
	return (
		<section>
			<div className={cn(styles['pop-card'], className)}>
				...
				<AdminPageHead>Популярный товар</AdminPageHead>
