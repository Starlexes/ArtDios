...
function EditWorkingHours({ className }: EditWorkingHoursProps) {
	const { isLoading, workingHours } = useAppSelector(
		(state: RootState) => state.workingHours
	);
	...
	return (
		<section>
			<div className={cn(styles['hours-items'], className)}>
				...
				<AdminPageHead>Время работы</AdminPageHead>
